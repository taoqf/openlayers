...
import { applyTransform, Extent } from './extent';
import { modulo } from './math';
import { fromEPSG4326, PROJECTIONS as EPSG3857_PROJECTIONS, toEPSG4326 } from './proj/epsg3857';
import { PROJECTIONS as EPSG4326_PROJECTIONS } from './proj/epsg4326';
import Projection from './proj/Projection';
import * as projections from './proj/projections';
import { add as addTransformFunc, clear as clearTransformFuncs, get as getTransformFunc } from './proj/transforms';
...
export type TransformFunction = (coordinates: number[], output?: number[], dimension?: number) => number[];
 ...
 *     values).
 */
export function cloneTransform(input: number[], opt_output?: number[], _opt_dimension?: number) {
	let output;
 ...
 */
export function identityTransform(input: number[], opt_output?: number[], _opt_dimension?: number) {
 ...
 */
export function addProjections(projs: Projection[]) {
	projs.forEach(addProjection);
}
 ...
 * @api
 */
export function get(projectionLike: ProjectionLike) {
	let projection = null;
 ...
 * @api
 */
export function getPointResolution(proj: ProjectionLike, resolution: number, point: Coordinate, opt_units?: Units) {
	...
	if (getter) {
		...
		if (units === Units.DEGREES && !opt_units || opt_units === Units.DEGREES) {
			...
			const to_EPSG4326 = getTransformFromProjections(projection!, get('EPSG:4326')!);
			let vertices = [
			...
			vertices = to_EPSG4326(vertices, vertices, 2);
			const width = getDistance(vertices.slice(0, 2) as Coordinate, vertices.slice(2, 4) as Coordinate);
			const height = getDistance(vertices.slice(4, 6) as Coordinate, vertices.slice(6, 8) as Coordinate);
 ...
 * @api
 */
export function addEquivalentProjections(proj: Projection[]) {
	addProjections(proj);
	proj.forEach((source) => {
		proj.forEach((destination) => {
 ...
 */
export function addEquivalentTransforms(projections1: Projection[], projections2: Projection[], forwardTransform: TransformFunction, inverseTransform: TransformFunction) {
 ...
 */
export function createProjection(projection: ProjectionLike, defaultCode: string) {
	if (!projection) {
 ...
 */
export function createTransformFromCoordinateTransform(coordTransform: (coordinate: Coordinate) => Coordinate) {
	return (
		/**
		 ...
		 */
		(input: number[], opt_output?: number[], opt_dimension?: number) => {
 ...
 * @api
 */
export function addCoordinateTransforms(source: ProjectionLike, destination: ProjectionLike, forward: (coordinate: Coordinate) => Coordinate, inverse: (coordinate: Coordinate) => Coordinate) {
 ...
 * @api
 */
export function fromLonLat(coordinate: Coordinate, opt_projection?: ProjectionLike) {
	return transform(coordinate, 'EPSG:4326',
		opt_projection !== undefined ? opt_projection : 'EPSG:3857') as Coordinate;
}
 ...
 * @api
 */
export function toLonLat(coordinate: Coordinate, opt_projection?: ProjectionLike) {
 ...
 * @api
 */
export function equivalent(projection1: Projection, projection2: Projection) {
 ...
 * @return {module:ol/proj~TransformFunction} Transform function.
 */
export function getTransformFromProjections(sourceProjection: Projection, destinationProjection: Projection) {
 ...
 * @api
 */
export function getTransform(source: ProjectionLike, destination: ProjectionLike) {
 ...
 * @api
 */
export function transform(coordinate: Coordinate, source: ProjectionLike, destination: ProjectionLike) {
	const transformFunc = getTransform(source, destination);
	return transformFunc(coordinate, undefined, coordinate.length) as Coordinate;
}
 ...
 * @api
 */
export function transformExtent(extent: Extent, source: ProjectionLike, destination: ProjectionLike) {
	const transformFunc = getTransform(source, destination);
 ...
 */
export function transformWithProjections(point: Coordinate, sourceProjection: Projection, destinationProjection: Projection) {
