...
import { transformWithOptions, ReadOptions } from '../format/Feature';
import XMLFeature from '../format/XMLFeature';
import { readDecimal, readBoolean, readString, writeStringTextNode, writeCDATASection, writeDecimalTextNode, writeBooleanTextNode } from '../format/xsd';
 ...
 */
let DEFAULT_FILL_STYLE: Fill = null!;
 ...
 */
let DEFAULT_IMAGE_STYLE_ANCHOR: Size;

/**
 * @type {module:ol/style/IconAnchorUnits}
 */
let DEFAULT_IMAGE_STYLE_ANCHOR_X_UNITS: IconAnchorUnits;

/**
 * @type {module:ol/style/IconAnchorUnits}
 */
let DEFAULT_IMAGE_STYLE_ANCHOR_Y_UNITS: IconAnchorUnits;

/**
 * @type {module:ol/size~Size}
 */
let DEFAULT_IMAGE_STYLE_SIZE: Size;

/**
 * @type {string}
 */
let DEFAULT_IMAGE_STYLE_SRC: string;

/**
 * @type {number}
 */
let DEFAULT_IMAGE_SCALE_MULTIPLIER: number;

/**
 * @type {module:ol/style/Image}
 */
let DEFAULT_IMAGE_STYLE: ImageStyle = null!;
 ...
 * @type {string}
 */
let DEFAULT_NO_IMAGE_STYLE: string;

/**
 * @type {module:ol/style/Stroke}
 */
let DEFAULT_STROKE_STYLE: Stroke = null!;
 ...
 * @type {module:ol/style/Stroke}
 */
let DEFAULT_TEXT_STROKE_STYLE: Stroke;

/**
 * @type {module:ol/style/Text}
 */
let DEFAULT_TEXT_STYLE: Text = null!;
 ...
 */
let DEFAULT_STYLE_ARRAY: Style[] = null!;
...
export default class KML extends XMLFeature {
	 ...
	 * @api
	 */
	public readFeature(source: Document | Node | object | string, opt_options?: ReadOptions) {
		return super.readFeature(source, opt_options);
	}
	 ...
	 * @api
	 */
	public readFeatures(source: Document | Node | object | string, opt_options?: ReadOptions) {
	...
	public readFeaturesFromNode(node: Node, opt_options?: Partial<ReadOptions>): Feature[] {
		...
		if (localName == 'Document' || localName == 'Folder') {
			const features = this.readDocumentOrFolder_(
				node, [this.getReadOptions(node, opt_options)]);
			if (features) {
				return features;
			} else {
	 ...
	 * @api
	 */
	public readName(source: Document | Node | string) {
		if (isDocument(source)) {
			return this.readNameFromDocument(/** @type {Document} */(source as Document));
		} else if (isNode(source)) {
			return this.readNameFromNode(/** @type {Node} */(source as Node));
	 ...
	 * @return {string|undefined} Name.
	 */
	public readNameFromDocument(doc: Document) {
	 ...
	 * @api
	 */
	public readNetworkLinks(source: Document | Node | string) {
		const networkLinks: any[] = [];
		if (isDocument(source)) {
			extend(networkLinks, this.readNetworkLinksFromDocument(
			/** @type {Document} */(source as Document)));
		} else if (isNode(source)) {
			extend(networkLinks, this.readNetworkLinksFromNode(
			/** @type {Node} */(source as Node)));
		} else if (typeof source === 'string') {
			const doc = parse(source);
	 ...
	 * @return {Array.<Object>} Network links.
	 */
	public readNetworkLinksFromDocument(doc: Document) {
		const networkLinks: any[] = [];
		for (let n = doc.firstChild; n; n = n.nextSibling) {
			if (n.nodeType == Node.ELEMENT_NODE) {
	 ...
	 * @api
	 */
	public readRegion(source: Document | Node | string) {
		const regions: any[] = [];
		if (isDocument(source)) {
	 ...
	 */
	public readRegionFromDocument(doc: Document) {
		const regions: any[] = [];
		for (let n = doc.firstChild; n; n = n.nextSibling) {
			if (n.nodeType == Node.ELEMENT_NODE) {
