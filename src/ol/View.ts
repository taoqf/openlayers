...
import { createExtent, none as centerNone, Type as CenterconstraintType } from './centerconstraint';
import { add as addCoordinate, Coordinate, equals as coordinatesEqual, rotate as rotateCoordinate } from './coordinate';
import { inAndOut } from './easing';
...
import { createProjection, METERS_PER_UNIT, ProjectionLike } from './proj';
import Projection from './proj/Projection';
import Units from './proj/Units';
import { createSnapToPower, createSnapToResolutions } from './resolutionconstraint';
import { Type as ResolutionconstraintType } from './resolutionconstraint';
import { createSnapToN, createSnapToZero, disable, none as rotationNone, Type as RotationconstraintType } from './rotationconstraint';
...
export default class View extends BaseObject {
	private hints_: Coordinate;
	private animations_: Animation[][];
	private updateAnimationKey_: number | undefined;
	private projection_: Projection | null;
	private maxResolution_ = 0;
	private minResolution_ = 0;
	private zoomFactor_ = 0;
	private resolutions_ = [0];
	private minZoom_ = 0;
	private constraints_: Constraints = null!;
	private options_: Partial<ViewOptions> = null!;
	constructor(opt_options?: Partial<ViewOptions>) {
		super();
	 ...
	 * @api
	 */
	public animate(...var_args: Array<Partial<AnimationOptions> | ((b: boolean) => void)>) {
		let animationCount = var_args.length;
		const callback = (() => {
			if (animationCount > 1 && typeof var_args[animationCount - 1] === 'function') {
				--animationCount;
				return var_args[animationCount - 1] as (b: boolean) => void;
			} else {
				return undefined;
			}
		})();
		if (!this.isDef()) {
			...
			const state = var_args[animationCount - 1] as AnimationOptions;
			if (state.center) {
		...
		let start = Date.now();
		let center = this.getCenter().slice() as Coordinate;
		let resolution = this.getResolution();
		let rotation = this.getRotation();
		const series = [];
		for (let i = 0; i < animationCount; ++i) {
			const options = /** @type {module:ol/View~AnimationOptions} */ (var_args[i] as AnimationOptions);
	 ...
	 */
	public calculateCenterRotate(rotation: number, anchor: Coordinate) {
		let center;
		const currentCenter = this.getCenter();
		if (currentCenter !== undefined) {
			center = [currentCenter[0] - anchor[0], currentCenter[1] - anchor[1]] as Coordinate;
	 ...
	 */
	public calculateCenterZoom(resolution: number, anchor: Coordinate) {
		let center;
		const currentCenter = this.getCenter();
	 ...
	 */
	public getSizeFromViewport_() {
		const size = [100, 100] as Coordinate;
	 ...
	 * @api
	 */
	public constrainCenter(center: Coordinate) {
	 ...
	 * @api
	 */
	public constrainResolution(resolution: number | undefined, opt_delta?: number, opt_direction?: number) {
	 ...
	 * @api
	 */
	public constrainRotation(rotation: number | undefined, opt_delta?: number) {
		const delta = opt_delta || 0;
	...
	public getCenter() {
		return (
			/** @type {module:ol/coordinate~Coordinate|undefined} */ (this.get(ViewProperty.CENTER) as Coordinate)
		);
	}
	 ...
	 */
	public getHints(opt_hints?: number[]) {
	 ...
	 * @api
	 */
	public calculateExtent(opt_size?: Size) {
	 ...
	 * @api
	 */
	public getResolutionForExtent(extent: Extent, opt_size?: Size) {
		const size = opt_size || this.getSizeFromViewport_();
	 ...
	 */
	public getResolutionForValueFunction(opt_power?: number) {
	 ...
	 */
	public getValueForResolutionFunction(opt_power?: number) {
		const power = opt_power || 2;
		const maxResolution = this.maxResolution_;
		const minResolution = this.minResolution_;
		const max = Math.log(maxResolution / minResolution) / Math.log(power);
	...
	public getState() {
		...
		return (
				...
				center: center.slice() as Coordinate,
	 ...
	 * @api
	 */
	public getZoomForResolution(resolution: number) {
	 ...
	 * @api
	 */
	public fit(geometryOrExtent: SimpleGeometry | Extent, opt_options?: FitOptions) {
		const options = opt_options || {} as FitOptions;
		...
		const center = [centerX, centerY] as Coordinate;
	 ...
	 * @api
	 */
	public centerOn(coordinate: Coordinate, size: Size, position: Pixel) {
	 ...
	 * @api
	 */
	public rotate(rotation: number, opt_anchor?: Coordinate) {
	 ...
	 * @api
	 */
	public setCenter(center: Coordinate | undefined) {
	 ...
	 */
	public setHint(hint: ViewHint, delta: number) {
	 ...
	 * @api
	 */
	public setResolution(resolution: number | undefined) {
		this.set(ViewProperty.RESOLUTION, resolution);
		if (this.getAnimating()) {
			this.cancelAnimations();
		}
	 ...
	 */
	private getUpdatedOptions_(newOptions: Partial<ViewOptions>) {
	 ...
	 */
	private applyOptions_(options: Partial<ViewOptions>) {

		/**
		 * @type {Object.<string, *>}
		 */
		const properties = {} as { [s: string]: any; };
		...
		const centerConstraint = createCenterConstraint(options) as CenterconstraintType;
 ...
 */
export function createCenterConstraint(options: Partial<ViewOptions>) {
 ...
 */
export function createResolutionConstraint(options: Partial<ViewOptions>) {
 ...
 */
export function createRotationConstraint(options: Partial<ViewOptions>) {
 ...
 */
export function isNoopAnimation(animation: Animation) {
