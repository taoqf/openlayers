 ...
 * @api
 */
export function getDistance(c1: Coordinate, c2: Coordinate, opt_radius?: number) {
 ...
 */
function getLengthInternal(coordinates: Coordinate[], radius: number) {
	let length = 0;
 ...
 */
function getAreaInternal(coordinates: Coordinate[], radius: number) {
	let area = 0;
 ...
 */
export function offset(c1: Coordinate, distance: number, bearing: number, opt_radius?: number) {
	const radius = opt_radius || DEFAULT_RADIUS;
	const lat1 = toRadians(c1[1]);
