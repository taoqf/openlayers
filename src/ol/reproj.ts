...
import { containsCoordinate, createEmpty, extend, Extent, getHeight, getTopLeft, getWidth } from './extent';
 ...
 */
export function calculateSourceResolution(sourceProj: Projection, targetProj: Projection, targetCenter: Coordinate, targetResolution: number) {
 ...
 */
function enlargeClipPoint(centroidX: number, centroidY: number, x: number, y: number) {
 ...
 */
export function render(width: number, height: number, pixelRatio: number, sourceResolution: number, _sourceExtent: Extent, targetResolution: number, targetExtent: Extent, triangulation: Triangulation, sources: Array<{ extent: Extent; image: HTMLCanvasElement | HTMLImageElement | HTMLVideoElement; }>, gutter: number, opt_renderEdges?: boolean) {
	// todo sourceExtent is not used
	...
	triangulation.getTriangles().forEach((triangle) => {
