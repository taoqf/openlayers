...
export type LoadFunction = (tile: Tile, s: string) => void;
...
export default abstract class Tile extends EventTarget {
	 ...
	 */
	public refreshInterimChain() {
		let tile = this.interimTile;
		if (!tile) {
			return;
		}

		let prev = this as Tile;
	 ...
	 */
	public getAlpha(id: number, time: number) {
		if (!this.transition_) {
			return 1;
		}
	 ...
	 * @param {number} id An id for the renderer.
	 */
	public endTransition(id: number) {
		if (this.transition_) {
